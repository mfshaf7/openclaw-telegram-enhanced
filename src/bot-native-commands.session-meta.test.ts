...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  it("suppresses duplicate assistant approval prose for native commands when buttons are available", async () => {
    replyMocks.dispatchReplyWithBufferedBlockDispatcher.mockImplementationOnce(
      async ({ dispatcherOptions }: DispatchReplyWithBufferedBlockDispatcherParams) => {
        await dispatcherOptions.deliver(
          {
            text: "To proceed, please approve this command:\n- /approve 7f423fdc allow-once\n- /approve 7f423fdc allow-always\n- /approve 7f423fdc deny",
          },
          { kind: "final" },
        );
        return dispatchReplyResult;
