  ...
  if (commandsToRegister.length > 0 || pluginCatalog.commands.length > 0) {
    if (typeof (bot as unknown as { command?: unknown }).command !== "function") {
      ...
      for (const command of nativeCommands) {
        ...
        bot.command(normalizedCommandName, async (ctx: TelegramNativeCommandContext) => {
          ...
          const deliveryState = {
            delivered: false,
            skippedNonSilent: 0,
            sawSuppressedExecApprovalPrompt: false,
          };
            ...
            dispatcherOptions: {
              ...
              deliver: async (payload, _info) => {
                if (
                  ...
                  deliveryState.sawSuppressedExecApprovalPrompt = true;
                  deliveryState.delivered = true;
                  return;
                }
                if (
                  deliveryState.sawSuppressedExecApprovalPrompt &&
      ...
      for (const localCommand of localCommands) {
        bot.command(localCommand.command, async (ctx: TelegramNativeCommandContext) => {
          const msg = ctx.message;
          if (!msg) {
            return;
          }
          ...
          await deliverReplies({
            ...
            ...deliveryBaseOptions,
            silent: runtimeTelegramCfg.silentErrorReplies === true && reply.isError === true,
          });
        });
      }
    }
