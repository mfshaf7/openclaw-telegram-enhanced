...
export function shouldSuppressLocalTelegramExecApprovalPrompt(params: {
  ...
  return looksLikeTelegramExecApprovalFallbackText(text);
}

export function looksLikeTelegramExecApprovalFallbackText(text: string): boolean {
  ...
  return (
    ...
    normalized.includes("you can approve") ||
    normalized.includes("to proceed") ||
    normalized.includes("replying with one of these commands") ||
    normalized.includes("reply with one of these commands")
  );
}
