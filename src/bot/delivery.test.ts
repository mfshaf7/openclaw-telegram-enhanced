...
describe("deliverReplies", () => {
  ...
  it("sends screenshot-style image replies as documents when telegram forceDocument is set", async () => {
    const runtime = createRuntime();
    ...
    mockMediaLoad("desktop-screenshot.png", "image/png", "image");

    await deliverWith({
      replies: [
        {
          mediaUrl: "/tmp/desktop-screenshot.png",
  ...
  it("upgrades MEDIA-prefixed local markdown file references into Telegram document sends", async () => {
    const runtime = createRuntime();
    const sendDocument = vi.fn().mockResolvedValue({
      message_id: 42,
      chat: { id: "123" },
