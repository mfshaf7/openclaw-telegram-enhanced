...
describe("telegram exec approvals", () => {
  ...
  it("suppresses fallback approval prose when inline approval buttons are available", () => {
    const cfg = buildConfig({ enabled: true, approvers: ["123"], target: "dm" });
    ...
    expect(
      shouldSuppressLocalTelegramExecApprovalPrompt({
        ...
        to: "123",
        payload: {
          text: "To proceed, please approve this command by replying with one of these commands:\n- /approve abcd1234 allow-once\n- /approve abcd1234 allow-always\n- /approve abcd1234 deny",
        },
      }),
    ).toBe(true);
  });

  it("does not suppress fallback approval prose when inline approval buttons are unavailable", () => {
    const cfg = buildConfig({ enabled: true, approvers: ["123"], target: "dm" });
